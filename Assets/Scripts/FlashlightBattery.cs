...
public class FlashlightBattery : MonoBehaviour
{
    ...
    public float maxBattery = 200f;  // Battery drains in 20 seconds
    public float batteryLife;  // Current battery life
    public float drainRate = 0f;  // Battery drains at 1 unit per second
    ...
    void Update()
    {
        ...
        {
            batteryLife -= Time.deltaTime; // Decrease battery over time

            // Adjust the flashlight intensity based on the remaining battery life
            float intensity = Mathf.Lerp(minIntensity, maxIntensity, batteryLife / maxBattery);
            flashlight.intensity = intensity;  // Set the flashlight intensity
